...
# 円を書く
def write_circle(center_x, center_y, angle, circle_size=0.1):#人の大きさは半径15cm
    # 初期化
    circle_x = [] #位置を表す円のx
    circle_y = [] #位置を表す円のy
...
class Animation_robot():
    ...
    def func_anim_plot(self, traj_x, traj_y, traj_th, traj_paths, traj_g_x, traj_g_y, traj_opt, obstacles):
        ...
        # goalを追加
        self.img_goal, = self.axis.plot([], [], '*', color='b', markersize=5)
        ...
        animation = ani.FuncAnimation(self.fig, self._update_anim, interval=100, frames=len(traj_g_x))
    ...
    def _update_anim(self, i):
        ...
        self.step_text.set_text("x:"+str(round(self.traj_x[i],3))+" y:"+str(round(self.traj_y[i],3)))
        ...
        Animation_robot.list_x = self.traj_x
        Animation_robot.list_y = self.traj_y
        return self.dwa_imgs
        
    def getlist():
        return Animation_robot.list_x,Animation_robot.list_y