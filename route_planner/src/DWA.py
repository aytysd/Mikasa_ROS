...
sys.path.append("home/ドキュメント/RouteGeneration/src/")
# 基本関数
# 正規化
...
class Const_goal():# goal作成プログラム
    def __init__(self):
        ...
        self.traj_g_x = []
        self.traj_g_y = []
        self.list_x,self.list_y,self.list_th,self.list_v = my_functions.MyFunctions.get_list()

    def calc_goal(self,count): # 本当は人の値が入ってもよいかも
        g_x = self.list_x[count]
        g_y = self.list_y[count]
...
class Main_controller():# Mainの制御クラス
    def __init__(self):
        ...
        list_Ob_x,list_Ob_y  = my_functions.MyFunctions.get_Ob_data()
        # 障害物（本当はレーザーの値等を使用）
        self.obstacles = []
        self.size = 0.25
        for i in range(len(list_Ob_x)):
            self.obstacles.append(Obstacle(list_Ob_x[i],list_Ob_y[i],self.size))
    ...
    def run_to_goal(self):
        ...
        time_step = 0
        count = 0
        list_x,list_y,list_th,list_v= my_functions.MyFunctions.get_list()
        while not goal_flag:

            g_x, g_y = self.goal_maker.calc_goal(count)
            # 入力決定
